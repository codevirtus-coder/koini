...
import { useCreatePaymentRequest, usePaymentRequestStatus } from '../../hooks/usePayment';
...
export default function RequestPaymentPage(): JSX.Element {
  ...
  const statusQuery = usePaymentRequestStatus(requestId, !!requestId && !isPending);

  const selectedRoute = routes?.find((r) => r.routeId === routeId);

  const handleSend = async () => {
    if (isPending) return;
    const res = await createRequest.mutateAsync({ passengerPhone: phone, amountKc: amount, routeId: routeId || undefined });
  ...
  if (statusQuery.data?.status === 'DECLINED' || statusQuery.data?.status === 'EXPIRED') {
    return (
      <PaymentStatusBanner
        variant="failure"
  ...
  if (requestId) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center gap-3">
        ...
        <h2 className="text-xl font-bold text-text-primary">Waiting for client to approve...</h2>
        <div className="text-sm text-text-secondary">{phone}</div>
        <div className="text-lg font-semibold text-text-primary">{formatKc(amount)} ({formatUsd(amount)})</div>
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-text-primary">Request Payment</h1>
      <Input label="Client Phone" value={phone} onChange={(e) => setPhone(e.target.value)} />
      <Select
        label="Route"
        value={routeId}
        onChange={(e) => {
        ...
        options={[{ label: 'Select route', value: '' }, ...(routes || []).map((r) => ({ label: `${r.name} (${formatKc(r.fareKc)})`, value: r.routeId }))]}
      />
      {selectedRoute && (
      ...
      <Button fullWidth isLoading={createRequest.isPending} onClick={handleSend}>
        Send Request
      </Button>
    </div>
  );
